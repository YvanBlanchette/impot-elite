...
export default function ClientPage() {
	...
	function buildFormData(c) {
		return {
			...
			sourceAcquisition: c.sourceAcquisition ?? "",
			statutFiscal: c.statutFiscal ?? "",
			occupation: c.occupation ?? "",
			employeur: c.employeur ?? "",
			nbrEnfants: c.nbrEnfants ?? "",
			niveauPriorite: c.niveauPriorite ?? "normal",
			typesRevenus: c.typesRevenus ? c.typesRevenus.split(",") : [],
			revenuFourchette: c.revenuFourchette ?? "",
			lienDossierExterne: c.lienDossierExterne ?? "",
			consentement: c.consentement ?? false,
			dateNaissance: c.dateNaissance ?? "",
			nas: c.nas ?? "",
			noTps: c.noTps ?? "",
			noTvq: c.noTvq ?? "",
			etatCivil: c.etatCivil ?? "",
			conjointNom: c.conjointNom ?? "",
			conjointPrenom: c.conjointPrenom ?? "",
			conjointNas: c.conjointNas ?? "",
			conjointDateNaissance: c.conjointDateNaissance ?? "",
			notesCrm: c.notesCrm ?? "",
		};
	}
